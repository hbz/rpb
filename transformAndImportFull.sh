...
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rppd-import.flux IN_FILE=output-rppd-strapi.ndjson HOST=$HOST API_TOKEN=$API_TOKEN"

# Normdaten
# curl --request DELETE "http://$HOST:1337/api/rpb-authorities/[1-5]"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-sw-to-strapi.flux IN_FILE=RPB-Export_HBZ_SW.txt OUT_FILE=output-sw-strapi.ndjson"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-sw-import.flux IN_FILE=output-sw-strapi.ndjson HOST=$HOST API_TOKEN=$API_TOKEN"

# Titeldaten
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-titel-to-strapi.flux"
# curl --request DELETE "http://$HOST:1337/api/articles/[1-5]"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-titel-import.flux PICK=all_equal('type','u') PATH=articles HOST=$HOST API_TOKEN=$API_TOKEN"
# curl --request DELETE "http://$HOST:1337/api/independent-works/[1-5]"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-titel-import.flux PICK=all_equal('type','Monografie') PATH=independent-works HOST=$HOST API_TOKEN=$API_TOKEN"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-titel-import.flux PICK=all_equal('type','Band') PATH=independent-works HOST=$HOST API_TOKEN=$API_TOKEN"
sbt --java-home $JAVA_HOME "runMain rpb.ETL etl/rpb-titel-import.flux PICK=all_contain('type','Mehrt') PATH=independent-works HOST=$HOST API_TOKEN=$API_TOKEN"
